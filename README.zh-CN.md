...
- **让 Agent 自动化操作：** 请求稳定的 JSON 字段，使用 `--jq` 过滤结果，调用本地 API，或通过 `window.mmx` 操作 Web UI。
...
Graph View 存放在 `mmx.graph-views.json`。Agent 可以直接编辑这个文件、验证配置，并在不打开浏览器的情况下执行 View。Version 2 View 可以组合多条路径，选择 Node 上展示哪些 Field，还可以把中间 Object 收缩为 Derived Edge。
...
为了支持浏览器自动化与 UI 开发，`window.mmx` 暴露导航、状态检查、Graph 操作、编辑等高层能力，Agent 不必重复模拟底层点击过程。
...
- **Interfaces：** CLI、本地 JSON API、Web UI 与 `window.mmx` 浏览器自动化
